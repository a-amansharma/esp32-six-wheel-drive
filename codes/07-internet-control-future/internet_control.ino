...
bool doubleHornActive = false;
int doubleHornPhase = 0;
...
void startDoubleHorn() {
  continuousHorn = false;
  doubleHornActive = true;
  doubleHornPhase = 1;
  hornTimer = millis() + 80;
  hornOn();
}
...
void updateHorn() {
  if (continuousHorn) {
    if (millis() - lastHornCommand > hornSafetyTimeout) {
      continuousHorn = false;
      hornOff();
    }
    return;
  ...
  if (!doubleHornActive) return;
  if (millis() < hornTimer) return;

  if (doubleHornPhase == 1) {
    hornOff();
    doubleHornPhase = 2;
    hornTimer = millis() + 65;
  } else if (doubleHornPhase == 2) {
    hornOn();
    doubleHornPhase = 3;
    hornTimer = millis() + 80;
  } else {
    hornOff();
    doubleHornActive = false;
    doubleHornPhase = 0;
  }
}
