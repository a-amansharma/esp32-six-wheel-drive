...
const int rampStep = 8;              // smaller = smoother, bigger = faster
const int rampInterval = 18;         // milliseconds
...
void setMovement(char move) {
  enableDrivers();
  currentMove = move;

  if (move == 'F') {
    targetLeftSpeed = speedValue;
    targetRightSpeed = speedValue;
  }
  else if (move == 'B') {
    targetLeftSpeed = -speedValue;
    targetRightSpeed = -speedValue;
...
void updateTargetSpeedAfterSlider() {
  setMovement(currentMove);
}
...
void handleCmd() {
  ...
  if (m == "F") setMovement('F');
  else if (m == "B") setMovement('B');
  else if (m == "L") setMovement('L');
  else if (m == "R") setMovement('R');
  else setMovement('S');
